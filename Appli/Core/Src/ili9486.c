...
#define ILI9486_CMD_SWRESET 0x01U
#define ILI9486_CMD_SLPOUT  0x11U
#define ILI9486_CMD_DISPON  0x29U
#define ILI9486_CMD_CASET   0x2AU
#define ILI9486_CMD_PASET   0x2BU
#define ILI9486_CMD_RAMWR   0x2CU
#define ILI9486_CMD_MADCTL  0x36U
#define ILI9486_CMD_PIXFMT  0x3AU
#define ILI9486_CMD_INVON   0x21U

#define ILI9486_SPI_TIMEOUT_MS 100U
...
static void ILI9486_Select(void);
static void ILI9486_Unselect(void);
static void ILI9486_WriteCommand(uint8_t command);
static void ILI9486_WriteData(const uint8_t *data, uint16_t length);
static void ILI9486_WriteDataByte(uint8_t data);
static void ILI9486_SetAddressWindow(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1);
static const uint8_t *ILI9486_FindGlyph(char character);
...
bool ILI9486_Init(void)
{
  ...
  ILI9486_WriteCommand(ILI9486_CMD_SWRESET);
  HAL_Delay(120U);
  ILI9486_WriteCommand(ILI9486_CMD_SLPOUT);
  HAL_Delay(120U);

  ILI9486_WriteCommand(ILI9486_CMD_PIXFMT);
  ...
  ILI9486_FillScreen(ILI9486_COLOR_BLACK);
  return true;
}
...
{
  ILI9486_FillRect(0U, 0U, ILI9486_WIDTH, ILI9486_HEIGHT, color);
}
...
void ILI9486_FillRect(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint16_t color)
{
  ...
  if (x >= ILI9486_WIDTH || y >= ILI9486_HEIGHT || width == 0U || height == 0U)
  {
    return;
  }

  if ((uint32_t)x + width > ILI9486_WIDTH)
  {
    width = (uint16_t)(ILI9486_WIDTH - x);
  }

  if ((uint32_t)y + height > ILI9486_HEIGHT)
  {
    height = (uint16_t)(ILI9486_HEIGHT - y);
  }
  ...
  ILI9486_SetAddressWindow(x, y, (uint16_t)(x + width - 1U), (uint16_t)(y + height - 1U));
  ...
  while (pixel_count > 0U)
  {
    ...
    HAL_SPI_Transmit(&hspi5, buffer, (uint16_t)(chunk_pixels * 2U), ILI9486_SPI_TIMEOUT_MS);
...
void ILI9486_DrawString(uint16_t x, uint16_t y, const char *text, uint16_t foreground, uint16_t background, uint8_t scale)
{
  ...
  while (*text != '\0')
  {
    ...
    {
      ILI9486_DrawChar(cursor_x, y, *text, foreground, background, scale);
...
{
  uint8_t line_buffer[ILI9486_WIDTH * 2U];

  if (pixels == NULL || x >= ILI9486_WIDTH || y >= ILI9486_HEIGHT || width == 0U || height == 0U)
  {
    return;
  }

  if (width > ILI9486_WIDTH)
  {
    width = ILI9486_WIDTH;
  }
  ...
  for (uint16_t row = 0U; row < height; ++row)
  {
    ...
    HAL_SPI_Transmit(&hspi5, line_buffer, (uint16_t)(width * 2U), ILI9486_SPI_TIMEOUT_MS);
  }
  ILI9486_Unselect();
}
...
static void ILI9486_WriteCommand(uint8_t command)
{
  ...
  ILI9486_Select();
  HAL_SPI_Transmit(&hspi5, &command, 1U, ILI9486_SPI_TIMEOUT_MS);
  ILI9486_Unselect();
}
...
static void ILI9486_WriteData(const uint8_t *data, uint16_t length)
{
  HAL_GPIO_WritePin(LCD_DC_GPIO_Port, LCD_DC_Pin, GPIO_PIN_SET);
  ILI9486_Select();
  HAL_SPI_Transmit(&hspi5, (uint8_t *)data, length, ILI9486_SPI_TIMEOUT_MS);
  ILI9486_Unselect();
}

static void ILI9486_WriteDataByte(uint8_t data)
{
  ILI9486_WriteData(&data, 1U);
}
...
static void ILI9486_SetAddressWindow(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
  ...
  ILI9486_WriteCommand(ILI9486_CMD_CASET);
  data[0] = (uint8_t)(x0 >> 8);
  data[1] = (uint8_t)x0;
  data[2] = (uint8_t)(x1 >> 8);
  data[3] = (uint8_t)x1;
  ILI9486_WriteData(data, sizeof(data));

  ILI9486_WriteCommand(ILI9486_CMD_PASET);
  data[0] = (uint8_t)(y0 >> 8);
  data[1] = (uint8_t)y0;
  data[2] = (uint8_t)(y1 >> 8);
  data[3] = (uint8_t)y1;
  ...
  ILI9486_WriteCommand(ILI9486_CMD_RAMWR);
}
...
static void ILI9486_DrawChar(uint16_t x, uint16_t y, char character, uint16_t foreground, uint16_t background, uint8_t scale)
{
  ...
  if ((uint32_t)x + char_width > ILI9486_WIDTH)
  {
    char_width = (uint16_t)(ILI9486_WIDTH - x);
  }
  if ((uint32_t)y + char_height > ILI9486_HEIGHT)
  {
    char_height = (uint16_t)(ILI9486_HEIGHT - y);
  }
  ...
  ILI9486_SetAddressWindow(x, y, (uint16_t)(x + char_width - 1U), (uint16_t)(y + char_height - 1U));
  HAL_GPIO_WritePin(LCD_DC_GPIO_Port, LCD_DC_Pin, GPIO_PIN_SET);
  ILI9486_Select();
